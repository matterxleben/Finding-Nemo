...
torch.serialization.add_safe_globals([MultiLabelImageDataset])
...
class LinearModel(nn.Module):
    ...
    def forward(self, x):
        ...
        return x  # pass into linear layer to get output
        # If you're using MSELoss, no activation should be applied to the final layer.
...
def train():
    ...
    hidden_dim = min(input_dim // 2, 1024)  # Reduce hidden layer size

    print(f"Input dim: {input_dim}, Hidden dim: {hidden_dim}")

    model = LinearModel(input_dim, hidden_dim, 290)

    # define loss and optimizer
    # play w hyper param here
    ...
    torch.save(model.state_dict(), 'layered_base_model_weights.pth')
