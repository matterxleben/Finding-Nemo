...
state_dict_half = torch.load('layered_base_model_weights.pth')
    ...
    with torch.no_grad():
        for images, labels in loader:
            ...
            for i in range(labels.shape[0]):  # Iterate over batch
                relevant_labels = labels[i].nonzero(
                    as_tuple=True)[0]  # Get true label indices
                ...
                # Iterate through top-20 predictions
