...
CSV_LOG_PATH = "resnet_training_log.csv"


def create_model(num_classes=290, pretrained=True):
    # ResNet50 
    model = models.resnet18(weights=models.ResNet18_Weights.DEFAULT if pretrained else None)

    in_features = model.fc.in_features  # Corrected: Use .fc instead of .classifier
    
    # Replace the fully connected (fc) layer with a new one for the target number of classes
...
def train_and_evaluate(
    ...
    """
    Train ResNet for multi-label classification and evaluate with MAP@20.
...
def run_experiment(num_epochs, lr, device):
    ...
    model = create_model(num_classes=290, pretrained=True)
...
run_experiment(num_epochs=NUM_EPOCHS, lr=1e-3, device=device)